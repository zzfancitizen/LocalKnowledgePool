...
published:
created: 2026-04-19
description: "tmux source code. Contribute to tmux/tmux development by creating an account on GitHub."
tags:
  - "clippings"
---
## Getting started
...
- `C-b l` changes to the last window, which is the window that was the current window before the window that is now.
...
```bash
# 例如：左边上侧运行 htop，左边下侧运行 shell，右边运行 vim
tmux new-session -s mylayout 'htop' \; split-window -h 'vim' \; split-window -v
```
